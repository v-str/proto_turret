...
- Корпус напечатан на 3D-принтере (FreeCAD-модели и STL приложены)

**Как это работает при загрузке:**

1. Плата стартует с **выключенными** драйверами моторов (EN = HIGH).
2. Главный цикл пингует micro-ros-agent каждые 2 секунды.
3. Когда агент найден → создаётся нода `proto_turret_node`: подписчик команд, публикация
   телеметрии, сервисы калибровки и PID-параметров.
4. Второй поток (`Ros2TaskExecutor`) разбирает очередь команд и управляет турелью.
...
- ROS 2: библиотека micro-ROS собрана под Humble, хост-стек — Lyrical
