...
#define PID_SERVICE_PID_PARAMS \
  "/turret_pid_params"  // ROS-сервис применения PID-параметра
...
// ----------------------------------------------------------------------------
#define ROS_EXECUTOR_HANDLES 3  // подписка + сервис калибровки + сервис параметров
...
// Коэффициенты раздельные для каждой оси — механика привода (редуктор, трение,
// инерция) разная: pan ходит на ~250°, tilt на ~65°. Пока обе оси стартуют с
// одинаковыми значениями; развести их нужно тюнингом по каждой оси отдельно.
// Доступны в рантайме через motor_apply_pid_params() (MotorPidParams) —
// см. motor_control.h/c и сервис PidParams.srv (ros2_ws).
...
#define PAN_PID_SPEED_MAX_DEG_PER_S 100.0f
...
#define PAN_PID_SPEED_SMOOTH 0.3f
...
// скакать быстрее, чем PAN_PID_OUTPUT_RATE в секунду. 6.0 = 0..1 за ~170 мс.
#define PAN_PID_OUTPUT_RATE 6.0f
...
#define PAN_PID_CORRECTION_MAX_FRACTION 1.0f
#define PAN_PID_KP_DEFAULT 0.5f
#define PAN_PID_KI_DEFAULT 0.0f
#define PAN_PID_KD_DEFAULT 0.0f

// --- Tilt (вертикаль) ---
#define TILT_PID_SPEED_MAX_DEG_PER_S 100.0f
#define TILT_PID_SPEED_SMOOTH 0.3f
#define TILT_PID_OUTPUT_RATE 6.0f
#define TILT_PID_CORRECTION_MAX_FRACTION 1.0f
#define TILT_PID_KP_DEFAULT 0.5f
#define TILT_PID_KI_DEFAULT 0.0f
#define TILT_PID_KD_DEFAULT 0.0f
